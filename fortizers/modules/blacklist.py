...
def blacklist(update, context):
    ...
    filter_list = tl(
        update.effective_message, "<b>Kata daftar hitam saat ini di {}:</b>\n"
    ).format(chat_name)
    ...
    for text in split_text:
        if filter_list == tl(
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Tidak ada pesan daftar hitam di <b>{}</b>!",
                ).format(chat_name),
...
Contoh nilai waktu: 4m = 4 menit, 3h = 3 jam, 6d = 6 hari, 5w = 5 minggu.""",
        ...
        if conn:
            text = tl(
                update.effective_message,
                "Mode blacklist diubah, Pengguna akan `{}` pada *{}*!",
            ).format(settypeblacklist, chat_name)
        else:
            text = tl(
                update.effective_message, "Mode blacklist diubah, Pengguna akan `{}`!"
        ...
        if conn:
            text = tl(
                update.effective_message,
                "Mode blacklist saat ini disetel ke *{}* pada *{}*.",
            ).format(settypeblacklist, chat_name)
        else:
            text = tl(
...
def __stats__():
    return tl(OWNER_ID, "{} pemicu daftar hitam, di seluruh {} obrolan.").format(
