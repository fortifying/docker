...
from fortizers.modules.helper_funcs.chat_status import (
    user_admin,
...
from fortizers.modules.helper_funcs.string_handling import (
    markdown_parser,
    ...
    try:
        ...
        if excp.message == "Button_url_invalid":
            ...
            try:
                ...
                if IS_DEBUG:
                    print(
                        "Cannot send welcome msg at {} ({})".format(chat.title, chat.id)
                    )
    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            # Don't welcome yourself
                ...
                context.bot.send_message(
                    chat.id, "Thanks for adding me into your group!."
                )
                ...
                if welc_type != sql.Types.TEXT and welc_type != sql.Types.BUTTON_TEXT:
                    ...
                    if is_user_ban_protected(
                        ...
                        if (
                            is_clicked.get(new_mem.id)
                            and is_clicked[new_mem.id] is True
                        ):
                        ...
                        if canrest:
                            if new_mem.id not in list(is_clicked):
                                ...
                                new_mem.id in list(is_clicked)
                                and is_clicked[new_mem.id] is False
                            ):
                                keyb.append(
                                    [
                                        InlineKeyboardButton(
                    ...
                    if is_user_ban_protected(
                        ...
                        if canrest:
                            if new_mem.id not in list(is_clicked):
                                ...
                                if timeout != "0":
                                    sql.add_to_timeout(
                                        chat.id, new_mem.id, int(timeout)
                                    )
                            elif (
                                new_mem.id in list(is_clicked)
                                and is_clicked[new_mem.id] == False
                            ):
                                if extra_verify:
                                    keyb.append(
                                        [
...
def check_bot_button(update, context):
    ...
    if int(user.id) != int(user_id):
        if IS_DEBUG:
            print("Not that user")
        query.answer(
            text=tl(update.effective_message, "Kamu bukan pengguna yang di tuju!")
        )
        return
    if getalluser.get(user.id) and getalluser.get(user.id) == True:
        query.answer(
            text=tl(
                update.effective_message, "Kamu sudah pernah mengklik ini sebelumnya!"
            )
        )
        return
    try:
    ...
    if welc_type != sql.Types.TEXT and welc_type != sql.Types.BUTTON_TEXT:
        # Formatting text
        first_name = (
            query.from_user.first_name or "PersonWithNoName"
        )  # edge case of empty name - occurs for some bugs.
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if OWNER_SPECIAL and left_mem.id == OWNER_ID:
                send_message(
                    update.effective_message,
                    tl(update.effective_message, "Selamat jalan master 😢"),
                )
                return
            ...
            if goodbye_type != sql.Types.TEXT and goodbye_type != sql.Types.BUTTON_TEXT:
                ...
                if left_mem.username:
                    ...
                    username = mention
                rules = (
                    "https://t.me/" + context.bot.username + "?start=" + str(chat.id)
                )
    ...
    if len(args) >= 1:
        ...
        if var == "yes" or var == "ya" or var == "on":
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Di nonaktifkan, saya tidak akan membisukan member masuk lagi",
                ),
            )
        else:
            send_message(
                update.effective_message,
                tl(update.effective_message, "Silakan tulis `on`/`ya`/`off`/`ga`!"),
                parse_mode=ParseMode.MARKDOWN,
            )
    else:
        (
...
def security_mute(update, context):
    ...
    if len(args) >= 1:
        ...
        if var[:1] == "0":
            mutetime = "0"
            sql.set_welcome_security(
                chat.id, getcur, extra_verify, "0", timeout, timeout_mode, cust_text
            )
            text = tl(
                update.effective_message,
            ...
            text = tl(
                update.effective_message,
                "Setiap member baru akan di bisukan selama {} sampai dia menekan tombol selamat datang!",
            ).format(var)
        ...
        if str(cur_value) == "0":
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Pengaturan saat ini: member baru akan di bisukan selama {} sampai dia menekan tombol selamat datang!",
                ).format(cur_value),
            )
...
def security_text(update, context):
    ...
    if len(args) >= 1:
        text = " ".join(args)
        sql.set_welcome_security(
            chat.id, getcur, extra_verify, cur_value, timeout, timeout_mode, text
        )
        text = tl(
            update.effective_message, "Tombol custom teks telah di ubah menjadi: `{}`"
        ).format(text)
        send_message(update.effective_message, text, parse_mode="markdown")
    else:
        send_message(
            update.effective_message,
            tl(
                update.effective_message, "Tombol teks security saat ini adalah: `{}`"
            ).format(cust_text),
            parse_mode="markdown",
        )
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if var == "no" or var == "off" or var == "tidak":
                ...
                send_message(
                    update.effective_message,
                    tl(update.effective_message, "Saya meninggalkan pesan layanan"),
                )
                ...
                send_message(
                    update.effective_message,
                    tl(
                        update.effective_message, "Saya akan membersihkan pesan layanan"
                    ),
                )
            else:
                send_message(
                    ...
                    tl(
                        update.effective_message,
                        "Silakan masukkan yes/ya atau no/tidak!",
                    ),
                    parse_mode=ParseMode.MARKDOWN,
                )
        else:
...
def welcome(update, context):
    ...
    if len(args) == 0 or args[0].lower() == "noformat":
        ...
        if cur_value[:1] == "0":
            welcsec += tl(
                update.effective_message,
                "(di bisukan selamanya sampai menekan tombol unmute)",
            )
        else:
            welcsec += tl(update.effective_message, "(di bisukan selama {})").format(
                cur_value
            )
        text = tl(
            update.effective_message,
        ...
        text += tl(
            update.effective_message, "Saat ini Saya menghapus layanan pesan: `{}`\n"
        ).format(cleanserv)
        text += tl(
            update.effective_message,
        ...
        text += tl(
            update.effective_message, "Pengguna baru harus verifikasi tombol: `{}`\n"
        ).format(
            tl(update.effective_message, "Aktif ")
            if extra_verify
            else tl(update.effective_message, "Tidak aktif ")
        )
        text += tl(
            update.effective_message, "Tombol welcomemute akan mengatakan: `{}`\n"
        ).format(cust_text)
        text += tl(
            update.effective_message,
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "Saya akan sopan 😁"),
            )
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "Aku ngambek, tidak menyapa lagi. 😣"),
            )
    ...
    if len(args) == 0 or args[0] == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Aku akan menyesal jika orang-orang pergi!",
                ),
            )
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Mereka pergi, mereka sudah mati bagi saya.",
                ),
            )
...
def set_welcome(update, context) -> str:
    ...
    if data_type is None:
        send_message(
            ...
            tl(
                update.effective_message,
                "Anda tidak menentukan apa yang harus dibalas!",
            ),
        )
        return ""
    ...
    send_message(
        update.effective_message,
        tl(update.effective_message, "Berhasil mengatur pesan sambutan kustom!"),
    )
...
def reset_welcome(update, context) -> str:
    ...
    send_message(
        ...
        tl(
            update.effective_message,
            "Berhasil menyetel ulang pesan sambutan ke default!",
        ),
    )
    return (
        "<b>{}:</b>"
...
def reset_goodbye(update, context) -> str:
    ...
    send_message(
        ...
        tl(
            update.effective_message,
            "Berhasil me-reset pesan selamat tinggal ke default!",
        ),
    )
    return (
        "<b>{}:</b>"
...
def clean_welcome(update, context):
    ...
    if not args:
        ...
        if clean_pref:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Saya *akan* menghapus pesan selamat datang hingga dua hari.",
                ),
                parse_mode="markdown",
            )
        else:
    ...
    if args[0].lower() in ("on", "yes"):
        ...
        send_message(
            ...
            tl(
                update.effective_message,
                "Saya hanya mengerti 'on/yes' or 'off/no' saja!",
            ),
        )
        return ""
...
def __chat_settings__(chat_id, user_id):
    ...
    ).format(welcome_pref, goodbye_pref, cleanserv)
...
def unwhChat(update, context):
    ...
    if args and len(args) == 1:
        ...
        try:
            ...
            update.effective_message.reply_text(
                "Chat has been successfully un-whitelisted!"
            )
...
SECURITY_HANDLER = CommandHandler(
    "welcomemute", security, pass_args=True, filters=Filters.group
)
SECURITY_MUTE_HANDLER = CommandHandler(
    "welcomemutetime", security_mute, pass_args=True, filters=Filters.group
)
SECURITY_BUTTONTXT_HANDLER = CommandHandler(
    "setmutetext", security_text, pass_args=True, filters=Filters.group
)
SECURITY_BUTTONRESET_HANDLER = CommandHandler(
    "resetmutetext", security_text_reset, filters=Filters.group
)
CLEAN_SERVICE_HANDLER = CommandHandler(
    "cleanservice", cleanservice, pass_args=True, filters=Filters.group
)
WHCHAT_HANDLER = CommandHandler(
    "whchat", whChat, pass_args=True, filters=Filters.user(OWNER_ID)
)
UNWHCHAT_HANDLER = CommandHandler(
    "unwhchat", unwhChat, pass_args=True, filters=Filters.user(OWNER_ID)
)

welcomesec_callback_handler = CallbackQueryHandler(
