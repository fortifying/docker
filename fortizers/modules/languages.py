...
def set_language(update, context):
    ...
    for x in LOADED_LANGS_ID:
        counter += 1
        tmp_list.append(
            InlineKeyboardButton(LANGS_TEXT[x], callback_data="set_lang({})".format(x))
        )
        if counter % 2 == 0:
    ...
    loaded_langs.append(
        [
            InlineKeyboardButton(
                "Translate",
                url="https://github.com/fortifying/fortizerswatcher/blob/master/TRANSLATION.md",
            )
        ]
    )
    ...
    send_message(
        ...
        tl(msg, "Bahasa di *{}* saat ini adalah:\n{}.\n\nPilih bahasa:").format(
            chatname, currlang
        ),
        parse_mode="markdown",
...
def button(update, context):
    ...
    if match:
        ...
        update.effective_message.edit_text(
            tl(query.message, "Bahasa telah di ubah ke {}!").format(
                LANGS_TEXT.get(set_lang)
            )
        )
