...
def add_to_timeout(chat_id, user_id, timeout_int):
    with TO_LOCK:
        user_filt = WelcomeTimeout(
            str(chat_id), user_id, int(time.time()) + int(timeout_int)
        )
...
def get_chat_timeout(chat_id):
    return (
        SESSION.query(WelcomeTimeout)
        .filter(WelcomeTimeout.chat_id == str(chat_id))
        .all()
    )
...
def welcome_security(chat_id):
    try:
        ...
        if security:
            return (
                security.security,
                security.extra_verify,
                security.mute_time,
                security.timeout,
                security.timeout_mode,
                security.custom_text,
            )
        else:
...
def set_welcome_security(
    chat_id, security, extra_verify, mute_time, timeout, timeout_mode, custom_text
):
    with WS_LOCK:
