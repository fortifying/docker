    ...
    if chat.type == chat.PRIVATE:
        if len(args) >= 1:
            ...
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "Preferensi laporan Anda saat ini: `{}`"
        ...
        if len(args) >= 1:
            ...
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "Pengaturan obrolan saat ini adalah: `{}`"
                ).format(sql.chat_should_report(chat.id)),
                parse_mode=ParseMode.MARKDOWN,
            )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    tl(update.effective_message, "⚠️ Pesan yang dilaporkan"),
                    url=chatlink,
                )
            ],
            [
                InlineKeyboardButton(
                    tl(update.effective_message, "⚠️ Tendang"),
                    callback_data="rp_{}=1={}".format(chat.id, reported_user.id),
                ),
                InlineKeyboardButton(
                    tl(update.effective_message, "⛔️ Banned"),
            ...
            [
                InlineKeyboardButton(
                    tl(update.effective_message, "Tutup Tombol"),
...
def report_alt(update, context) -> str:
    ...
    if chat and message.reply_to_message and sql.chat_should_report(chat.id):
        ...
        for admin in admin_list:
            ...
            if sql.user_should_report(admin.user.id):
                all_admins.append(
                    "<a href='tg://user?id={}'>⁣</a>".format(admin.user.id)
                )

        context.bot.send_message(
            chat.id,
            tl(
                update.effective_message, "⚠️ {} <b>telah di laporkan ke admin!</b>{}"
            ).format(
                mention_html(reported_user.id, reported_user.first_name),
                "".join(all_admins),
            ),
...
def button(update, context):
    ...
    if splitter[1] == "1":
        keyboard = [
            [
                InlineKeyboardButton(
                    tl(update.effective_message, "Ya"),
                    callback_data="ak_1+y|{}={}".format(report_chat, report_target),
                ),
                InlineKeyboardButton(
                    tl(update.effective_message, "Tidak"),
        ...
        if splitter[0] == "1":
            try:
                ...
                context.bot.sendMessage(
                    report_chat,
                    text=tl(
                        update.effective_message, "{} telah di tendang!\nOleh: {}"
                    ).format(
            ...
            try:
                ...
                context.bot.sendMessage(
                    report_chat,
                    text=tl(
                        update.effective_message, "{} telah di banned!\nOleh: {}"
                    ).format(
                        mention_markdown(userinfo["id"], userinfo["name"]),
                        mention_markdown(chat.id, chat.first_name),
                    ),
