...
def verify_welcome(update, context, chat_id):
        ...
        for y in range(3):
            ...
            if verbox[0] == y and ver1:
                ...
                linebox.append(
                    InlineKeyboardButton(
                        text=verify_emoji,
                        callback_data="verify_me(n|{}|{})".format(user_id, chat_id),
                    )
                )
    ...
    context.bot.send_photo(
        ...
        caption=tl(
            update.effective_message, "Tolong pilih emoji yang sama dibawah ini:"
        ),
...
def verify_button_pressed(update, context):
    ...
    if is_ok == "y":
        ...
        query.answer(
            text=tl(
                update.effective_message, "Berhasil! Anda dapat chatting di {} sekarang"
            ).format(chat_name),
            show_alert=True,
        )
    else:
...
verify_callback_handler = CallbackQueryHandler(
    verify_button_pressed, pattern=r"verify_me"
)
