...
def set_verify_welcome(update, context):
    ...
    if len(args) >= 1:
        ...
        ).format(getcur, extra_verify, cur_value, make_time(int(timeout)), cust_text)
        send_message(update.effective_message, text, parse_mode="markdown")
...
def set_welctimeout(update, context):
    ...
    if len(args) >= 1:
        ...
        if var[:1] == "0":
            mutetime = "0"
            sql.set_welcome_security(
                chat.id, getcur, extra_verify, cur_value, "0", timeout_mode, cust_text
            )
            text = tl(
                update.effective_message, "Batas waktu verifikasi telah di nonaktifkan!"
            )
        else:
        ...
        if timeout == "0":
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "Pengaturan batas waktu ketika join: *{}*"
                ).format("Disabled"),
                parse_mode="markdown",
            )
        else:
...
def timeout_mode(update, context):
    ...
    if args:
        if (
            args[0].lower() == "kick"
            or args[0].lower() == "tendang"
            or args[0].lower() == "leave"
        ):
            settypeblacklist = tl(update.effective_message, "kick")
            sql.set_welcome_security(
                chat.id, getcur, extra_verify, cur_value, timeout, 1, cust_text
            )
        elif args[0].lower() == "ban" or args[0].lower() == "banned":
            settypeblacklist = tl(update.effective_message, "banned")
            sql.set_welcome_security(
                chat.id, getcur, extra_verify, cur_value, timeout, 2, cust_text
            )
        else:
            send_message(
                update.effective_message,
                tl(update.effective_message, "Saya hanya mengerti kick/banned!"),
            )
            return
        if conn:
            text = tl(
                update.effective_message,
                "Mode timeout diubah, Pengguna akan di `{}` pada *{}*!",
            ).format(settypeblacklist, chat_name)
        else:
            text = tl(
                update.effective_message, "Mode timeout diubah, Pengguna akan di `{}`!"
        ...
        if conn:
            text = tl(
                update.effective_message,
                "Mode timeout saat ini disetel ke *{}* pada *{}*.",
            ).format(settypeblacklist, chat_name)
        else:
            text = tl(
                update.effective_message, "Mode timeout saat ini disetel ke *{}*."
            ).format(settypeblacklist)
...
WELCVERIFY_HANDLER = CommandHandler(
    "welcomeverify", set_verify_welcome, pass_args=True, filters=Filters.group
)
WELTIMEOUT_HANDLER = CommandHandler(
    "wtimeout", set_welctimeout, pass_args=True, filters=Filters.group
)
WELMODE_HANDLER = CommandHandler(
    "wtmode", timeout_mode, pass_args=True, filters=Filters.group
)
