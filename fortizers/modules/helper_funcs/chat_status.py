...
def user_can_promote(chat: Chat, user: User, bot_id: int) -> bool:
    return (
        chat.get_member(bot_id).can_promote_members
        and chat.get_member(user.id).can_promote_members
    )


def user_can_delete(chat: Chat, user: User, bot_id: int) -> bool:
    return (
        chat.get_member(bot_id).can_delete_messages
        and chat.get_member(user.id).can_delete_messages
    )
            ...
            if str(err) == "Reply message not found":
                ...
                for x in log_errors:
                    if " update.effective_message" in x:
                        do_func = x.split("update.effective_message", 1)[1].split(
                            ")", 1
                        )
