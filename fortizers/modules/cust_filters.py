...
def filters(update, context):
    ...
    if not msg.reply_to_message and len(extracted) >= 2:
        ...
        send_message(
            update.effective_message, tl(update.effective_message, "Invalid filter!")
        )
        return
    ...
    send_message(
        update.effective_message,
        tl(update.effective_message, "Handler '{}' ditambahkan di *{}*!").format(
            keyword, chat_name
        ),
        parse_mode=telegram.ParseMode.MARKDOWN,
    )
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Ya, saya akan berhenti menjawabnya di *{}*.",
                ).format(chat_name),
                parse_mode=telegram.ParseMode.MARKDOWN,
            )
            raise DispatcherHandlerStop
...
def __stats__():
    return tl(OWNER_ID, "{} filter, pada {} obrolan.").format(
