        ...
        if len(args) >= 1:
            ...
            if var == "no" or var == "tidak":
                ...
                send_message(
                    ...
                    languages.tl(
                        update.effective_message,
                        "Sambungan telah dinonaktifkan untuk obrolan ini",
                    ),
                )
                ...
                send_message(
                    ...
                    languages.tl(
                        update.effective_message,
                        "Koneksi di aktifkan untuk obrolan ini",
                    ),
                )
            else:
                send_message(
                    ...
                    languages.tl(
                        update.effective_message,
                        "Silakan masukkan `ya`/`yes` atau `tidak`/`no`!",
                    ),
...
def connection_chat(update, context):
    ...
    if conn:
        teks = languages.tl(
            update.effective_message, "Saat ini Anda terhubung dengan {}.\n"
        ).format(chat_name)
    else:
        teks = languages.tl(
            update.effective_message, "Saat ini Anda tidak terhubung dengan grup.\n"
        )
    ...
    if update.effective_chat.type == "private":
        if args and len(args) >= 1:
            try:
                ...
                try:
                    ...
                    send_message(
                        update.effective_message,
                        languages.tl(
                            update.effective_message, "ID Obrolan tidak valid!"
                        ),
                    )
                    return
            except error.BadRequest:
            ...
            if gethistory:
                buttons = [
                    InlineKeyboardButton(
                        text=languages.tl(update.effective_message, "❎ Close button"),
                        callback_data="connect_close",
                    ),
                    InlineKeyboardButton(
                        text=languages.tl(update.effective_message, "🧹 Hapus riwayat"),
            ...
            if conn:
                ...
                text = languages.tl(
                    update.effective_message, "Anda telah terkoneksi pada *{}* (`{}`)"
                ).format(connectedchat.title, conn)
                buttons.append(
                    InlineKeyboardButton(
                        text=languages.tl(
                            update.effective_message, "🔌 Putuskan sambungan"
                        ),
                ...
                text = languages.tl(
                    update.effective_message,
                    "Tulis ID obrolan atau tagnya untuk terhubung!",
                )
            if gethistory:
                text += languages.tl(
                    update.effective_message, "\n\n*Riwayat koneksi:*\n"
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            if connection_status:
                ...
                send_message(
                    update.effective_message,
                    languages.tl(
                        update.effective_message, "Berhasil tersambung ke *{}*."
                    ).format(chat_name),
                    parse_mode=ParseMode.MARKDOWN,
                )
                try:
                    ...
                    context.bot.send_message(
                        ...
                        languages.tl(
                            update.effective_message,
                            "Anda telah terhubung dengan *{}*. Gunakan /connection untuk informasi perintah apa saja yang tersedia.",
                        ).format(chat_name),
                        parse_mode="markdown",
                    )
                except BadRequest:
...
def disconnect_chat(update, context):
    if update.effective_chat.type == "private":
        ...
        send_message(
            update.effective_message,
            languages.tl(
                update.effective_message, "Penggunaan terbatas hanya untuk PM"
            ),
        )
...
def connect_button(update, context) -> str:
    ...
    if connect_match:
        ...
        query.message.edit_text(
            languages.tl(
                update.effective_message, "Riwayat yang terhubung telah dihapus!"
            )
        )
    elif connect_close:
        query.message.edit_text(
            languages.tl(
                update.effective_message, "Closed.\nTo open again, type /connect"
            )
        )
    else:
...
HELP_CONNECT_CHAT_HANDLER = CommandHandler(
    "helpconnect", help_connect_chat, pass_args=True
)
