...
def get(bot, update, notename, show_none=True, no_format=False):
    ...
    if note:
        ...
        if note.is_reply:
            ...
            try:
                ...
                if excp.message == "Entity_mention_user_invalid":
                    ...
                    LOGGER.exception(
                        "Could not parse message #%s in the chat %s",
                        notename,
                        str(chat_id),
                    )
        ...
        send_message(
            update.effective_message,
            tl(update.effective_message, "This note doesn't exist"),
        )
...
def cmd_get(update, context):
    ...
    if len(args) >= 2 and args[1].lower() == "noformat":
        ...
        send_message(
            update.effective_message, tl(update.effective_message, "Get what?")
        )
...
def save(update, context):
    ...
    if data_type is None:
        send_message(
            update.effective_message, tl(update.effective_message, "Tidak ada catatan!")
        )
        return
    ...
    if conn:
        savedtext = tl(
            update.effective_message,
            "Ok, catatan `{note_name}` disimpan di *{chat_name}*.",
        ).format(note_name=note_name, chat_name=chat_name)
    else:
        savedtext = tl(
            update.effective_message, "Ok, catatan `{note_name}` disimpan."
    ...
    try:
        ...
        if conn:
            ...
            savedtext = tl(
                update.effective_message,
                "Ok, catatan <code>{note_name}</code> disimpan.",
            ).format(note_name=note_name)
    ...
    if len(args) >= 1:
        ...
        if len(catatan) >= 1 and len(catatangagal) == 0:
            if conn:
                ...
                rtext = tl(
                    update.effective_message, "Catatan `{note_name}` dihapus 😁"
                ).format(note_name=", ".join(catatan))
            try:
                send_message(
                    update.effective_message, rtext, parse_mode=ParseMode.MARKDOWN
                )
            except BadRequest:
                if conn:
                    rtext = tl(
            ...
            if conn:
                ...
                ).format(chat_name=chat_name, fnote_name=", ".join(catatangagal))
            else:
                rtext = tl(
                    update.effective_message, "Catatan `{fnote_name}` gagal dihapus!"
    ...
    if len(args) >= 1:
        if args[0] in ("yes", "on"):
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Private Note was *disabled*, notes will be sent to group.",
                ),
                parse_mode="markdown",
            )
        else:
        ...
        send_message(
            update.effective_message,
            tl(
                update.effective_message, "Current Private Note settings at {}: *{}*{}"
            ).format(
                chat_name,
...
def list_notes(update, context):
    ...
    if msg == tl(update.effective_message, "*Notes on {}:*\n").format(
        chat_name
    ) or msg == tl(update.effective_message, "*Local notes:*\n"):
        if conn:
            send_message(
                update.effective_message,
                tl(update.effective_message, "No notes in *{}*!").format(chat_name),
                parse_mode="markdown",
            )
        else:
            send_message(
                update.effective_message,
                tl(update.effective_message, "No notes in this chat!"),
            )
        ...
        try:
            ...
            for note in note_list:
                ...
                if len(msg) + len(note_name) > MAX_MESSAGE_LENGTH:
                    send_message(
                        update.effective_message, msg, parse_mode=ParseMode.MARKDOWN
                    )
                    msg = ""
                msg += note_name
            msg += tl(
...
def remove_all_notes(update, context):
    ...
    if not note_list:
        message.reply_text(
            tl(chat.id, "No notes in *{}*!").format(chat.title),
...
def __stats__():
    return tl(OWNER_ID, "`{}` notes, on `{}` chat.").format(
