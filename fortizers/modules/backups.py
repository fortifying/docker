...
def import_data(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if filetype.split(".")[-1] not in ("backup", "json", "txt"):
            send_message(
                update.effective_message,
                tl(update.effective_message, "File cadangan tidak valid!"),
            )
            return
        try:
            ...
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Coba unduh dan unggah ulang file seperti Anda sendiri sebelum mengimpor - yang ini sepertinya rusak!",
                ),
            )
            return
        ...
        try:
            ...
            if data.get("bot_base") == "Fortizers":
                ...
                if conn:
                    text = tl(
                        update.effective_message,
                        "Cadangan sepenuhnya dikembalikan pada *{}*. Selamat datang kembali! 😀",
                    ).format(chat_name)
                else:
                    text = tl(
                        update.effective_message,
        ...
        if len(data) > 1 and str(chat_id) not in data:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Ada lebih dari satu grup di file ini, dan tidak ada yang memiliki id obrolan yang sama dengan"
        ...
        try:
            if data.get(str(chat_id)) == None:
                if conn:
                    text = tl(
                        update.effective_message,
                        "Backup berasal chat lain, Saya tidak bisa mengembalikan chat lain kedalam chat *{}*",
                    ).format(chat_name)
                else:
                    text = tl(
                        update.effective_message,
            ...
            return send_message(
                ...
                tl(
                    update.effective_message,
                    "Telah terjadi error dalam pengecekan data, silahkan laporkan kepada pembuat saya "
        ...
        try:
            ...
            LOGGER.exception(
                "Impor untuk id chat %s dengan nama %s gagal.",
                str(chat_id),
                str(chat.title),
            )
            return
...
def export_data(update, context):
    ...
    try:
        ...
        pass
    send = context.bot.sendDocument(
        current_chat_id,
        document=open("{}-Fortizers.backup".format(chat_id), "rb"),
        caption=tl(
            update.effective_message,
