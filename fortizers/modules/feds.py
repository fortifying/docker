...
from fortizers.modules.helper_funcs.extraction import (
    extract_user,
    extract_unt_fedban,
    extract_user_fban,
)
...
def new_fed(update, context):
    ...
    if chat.type != "private":
        send_message(
            ...
            tl(
                update.effective_message,
                "Buat federasi Anda di PM saya, bukan dalam grup.",
            ),
        )
        return
    if len(message.text) == 1:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Tolong tulis nama federasinya!"),
        )
        return
...
def del_fed(update, context):
    ...
    if args:
        ...
        if getinfo is False:
            send_message(
                update.effective_message,
                tl(update.effective_message, "Federasi ini tidak di temukan!"),
            )
            return
        ...
        send_message(
            update.effective_message,
            tl(update.effective_message, "Apa yang harus saya hapus?"),
        )
        return
    ...
    send_message(
        ...
        tl(
            update.effective_message,
            "Anda yakin ingin menghapus federasi Anda? Tindakan ini tidak bisa dibatalkan, Anda akan kehilangan seluruh daftar larangan Anda, dan '{}' akan hilang secara permanen.",
        ).format(getinfo["fname"]),
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=tl(update.effective_message, "⚠️ Hapus Federasi ⚠️"),
                        callback_data="rmfed_{}".format(fed_id),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=tl(update.effective_message, "Batalkan"),
...
def fed_chat(update, context):
    ...
    if not fed_id:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Grup ini tidak dalam federasi apa pun!"),
        )
        return
    ...
    text = tl(
        update.effective_message, "Obrolan ini adalah bagian dari federasi berikut:"
    )
...
def join_fed(update, context):
    ...
    if len(args) >= 1:
        ...
        if get_fedlog:
            if eval(get_fedlog):
                context.bot.send_message(
                    ...
                    tl(
                        update.effective_message,
                        "Obrolan *{}* telah bergabung ke federasi *{}*",
                    ).format(chat.title, getfed["fname"]),
                    parse_mode="markdown",
                )
        ...
        send_message(
            ...
            tl(
                update.effective_message,
                "Obrolan ini telah bergabung dengan federasi {}!",
            ).format(getfed["fname"]),
        )
...
def leave_fed(update, context):
    ...
    if getuser in "creator" or user.id in SUDO_USERS:
            ...
            if get_fedlog:
                if eval(get_fedlog):
                    context.bot.send_message(
                        ...
                        tl(
                            update.effective_message,
                            "Obrolan *{}* telah keluar ke federasi *{}*",
                        ).format(chat.title, fed_info["fname"]),
                        parse_mode="markdown",
                    )
            send_message(
                update.effective_message,
                tl(
                    update.effective_message,
                    "Obrolan ini telah keluar dari federasi {}!",
                ).format(fed_info["fname"]),
            )
        else:
            send_message(
                update.effective_message,
        ...
        send_message(
            update.effective_message,
            tl(update.effective_message, "Hanya pembuat grup yang dapat melakukannya!"),
        )
...
def user_join_fed(update, context):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if res:
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "Gagal dipromosikan!"),
            )
    else:
        send_message(
            update.effective_message,
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if res is True:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "💔 Dia telah di keluarkan dari federasi anda!",
                ),
            )
        else:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Saya tidak bisa mengusirnya, Saya tidak berdaya!",
                ),
            )
    else:
        send_message(
...
def fed_info(update, context):
    ...
    text += tl(update.effective_message, "\nSeluruh admin: <code>{}</code>").format(
        TotalAdminFed
    )
...
def fed_ban(update, context):
    ...
    if not user_id:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Anda sepertinya tidak merujuk ke pengguna."),
        )
        return
    ...
    if is_user_fed_owner(fed_id, user_id) is True:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Mengapa Anda mencoba fban pemilik federasi?"),
        )
        return
    ...
    if int(user_id) in SUDO_USERS:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Saya tidak akan fban pengguna sudo!"),
        )
        return
    ...
    try:
        ...
        if not str(user_id).isdigit():
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "Itu bukan pengguna!"),
            )
            return
        isvalid = False
    ...
    if fban:
        ...
        starting = tl(
            update.effective_message,
            "Alasan fban di ganti untuk {} pada Federasi <b>{}</b>.",
        ...
        if not temp:
            send_message(
                update.effective_message,
                tl(update.effective_message, "Gagal mengupdate alasan fedban!"),
            )
            return
        x = sql.fban_user(
            fed_id,
        ...
        send_message(
            update.effective_message,
            tl(update.effective_message, "Alasan fedban telah di perbarui."),
        )
        return
    ...
    starting = tl(
        update.effective_message,
        "Memulai larangan federasi untuk {} pada Federasi <b>{}</b>.",
    ).format(user_target, fed_name)
    send_message(update.effective_message, starting, parse_mode=ParseMode.HTML)
    ...
    for fedschat in fed_chats:
        try:
            ...
            if excp.message in FBAN_ERRORS:
                ...
                LOGGER.warning(
                    "Tidak dapat fban di {} karena: {}".format(fedschat, excp.message)
                )
        except TelegramError:
            pass
    ...
    send_message(
        update.effective_message,
        tl(update.effective_message, "Orang ini telah di fbanned."),
    )
...
def unfban(update, context):
    ...
    if not fban:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Pengguna ini tidak di fbanned!"),
        )
        return
    ...
    send_message(
        ...
        tl(
            update.effective_message,
            "Saya akan memberi {} kesempatan kedua dalam federasi ini.",
        ).format(user_target),
        parse_mode="HTML",
    )
    ...
    for fedchats in chat_list:
        try:
            ...
            if excp.message in UNFBAN_ERRORS:
                ...
                LOGGER.warning(
                    "Tidak dapat fban di {} karena: {}".format(fedchats, excp.message)
                )
        except TelegramError:
            pass
    ...
    send_message(
        update.effective_message,
        tl(update.effective_message, "Orang ini telah di un-fbanned."),
    )
...
def set_frules(update, context):
    ...
    if not fed_id:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Obrolan ini tidak ada di federasi apa pun!"),
        )
        return
    ...
    if len(args) >= 1:
        ...
        if get_fedlog:
            if eval(get_fedlog):
                context.bot.send_message(
                    ...
                    tl(
                        update.effective_message,
                        "*{}* telah mengganti aturan federasi *{}*",
                    ).format(user.first_name, getfed["fname"]),
                    parse_mode="markdown",
                )
        send_message(
            update.effective_message,
            tl(update.effective_message, "Aturan telah di ganti menjadi:\n{}!").format(
                rules
            ),
        )
    else:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Silakan tulis aturan untuk mengaturnya!"),
        )
...
def fed_broadcast(update, context):
    ...
    if args:
        ...
        if failed >= 1:
            send_text += tl(
                update.effective_message,
                "{} grup gagal menerima pesan, mungkin karena meninggalkan federasi.",
            ).format(failed)
...
def fed_ban_list(update, context):
    ...
    if len(getfban) == 0:
        send_message(
            ...
            tl(
                update.effective_message,
                "Tidak ada pengguna yang di fban di federasi {}",
            ).format(info["fname"]),
            parse_mode=ParseMode.HTML,
        )
        return
        ...
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                ...
                update.effective_message.reply_document(
                    ...
                    caption=tl(
                        update.effective_message,
                        "Total {} pengguna terkena blokir federasi {}.",
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                ...
                caption=tl(
                    update.effective_message,
                    "Berikut adalah daftar pengguna yang saat ini difban pada federasi {}.",
                ).format(info["fname"]),
            )
...
def fed_notif(update, context):
    ...
    if args:
        if args[0] in ("yes", "on", "ya"):
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "Tolong masukan `ya`/`on`/`ga`/`off`"),
                parse_mode="markdown",
            )
    else:
        ...
        send_message(
            ...
            tl(
                update.effective_message,
                "Preferensi laporan federasi anda saat ini: `{}`",
            ).format(getreport),
            parse_mode="markdown",
        )
...
def fed_chats(update, context):
    ...
    if len(getlist) == 0:
        send_message(
            ...
            tl(
                update.effective_message,
                "Tidak ada obrolan yang bergabung pada federasi {}",
            ).format(info["fname"]),
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    text = tl(
        update.effective_message, "<b>Obrolan yang bergabung pada federasi {}:</b>\n"
    ).format(info["fname"])
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                ...
                caption=tl(
                    update.effective_message,
                    "Berikut adalah daftar obrolan yang bergabung federasi {}.",
                ).format(info["fname"]),
            )
...
def fed_import_bans(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if fileformat == "json":
            ...
            text = tl(
                update.effective_message,
                "Berkas blokir berhasil diimpor. {} orang diblokir.",
            ).format(success)
            if failed >= 1:
                text += tl(update.effective_message, " {} gagal di impor.").format(
                    failed
                )
            get_fedlog = sql.get_fed_log(fed_id)
            if get_fedlog:
                if eval(get_fedlog):
                    teks = tl(
                        update.effective_message,
                        "Federasi *{}* telah berhasil mengimpor data. {} di blokir",
                    ).format(getfed["fname"], success)
                    if failed >= 1:
                        teks += tl(
                            update.effective_message, " {} gagal di impor."
                        ).format(failed)
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "File tidak di dukung."),
            )
            return
    ...
    if getfed:
        ...
        if delete:
            query.message.edit_text(
                tl(
                    update.effective_message,
                    "Anda telah menghapus federasi Anda! Sekarang semua Grup yang terhubung dengan `{}` sekarang tidak memiliki federasi.",
                ).format(getfed["fname"]),
                parse_mode="markdown",
            )
    ...
    if user_id and user_id != "error":
        if len(args) == 2 and args[0].isdigit():
            ...
            if not user_name:
                send_message(
                    update.effective_message,
                    tl(
                        update.effective_message, "Federasi {} tidak di temukan!"
                    ).format(fed_id),
                    parse_mode="markdown",
                )
                return
            ...
            if not reason:
                send_message(
                    update.effective_message,
                    tl(
                        update.effective_message, "{} belum di larang di federasi ini!"
                    ).format(user_name),
                )
            else:
                teks = tl(
        ...
        if len(fbanlist) == 0:
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "{} belum di larang di federasi mana pun!"
                ).format(user_name),
            )
            return
        else:
            teks = tl(
                update.effective_message, "{} sudah di larang di federasi ini:\n"
            ).format(user_name)
            for x in fbanlist:
                teks += "- `{}`: {}\n".format(x[0], x[1][:20])
            teks += tl(
        ...
        if not fedinfo:
            send_message(
                update.effective_message,
                tl(update.effective_message, "Federasi {} tidak di temukan!").format(
                    fed_id
                ),
            )
            return
        ...
        if not reason:
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "{} tidak di larang di federasi ini"
                ).format(name),
            )
            return
        send_message(
...
def set_fed_log(update, context):
    ...
    if args:
        ...
        if not fedinfo:
            send_message(
                update.effective_message,
                tl(update.effective_message, "Federasi ini tidak ada!"),
            )
            return
        ...
        if setlog:
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "Log federasi `{}` telah di setel pada {}"
                ).format(fedinfo["fname"], chat.title),
                parse_mode="markdown",
            )
    else:
        send_message(
            update.effective_message,
            tl(update.effective_message, "Anda belum memberikan ID federasinya!"),
        )
    ...
    if args:
        ...
        if setlog:
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "Log federasi `{}` telah di cabut pada {}"
                ).format(fedinfo["fname"], chat.title),
                parse_mode="markdown",
            )
    else:
    ...
    if args:
        ...
        if subfed:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Federasi `{}` telah mengikuti federasi `{}`. Setiap ada fedban dari federasi tersebut, federasi ini juga akan banned pengguna tsb.",
            ...
            if get_fedlog:
                if int(get_fedlog) != int(chat.id):
                    context.bot.send_message(
                        ...
                        tl(
                            update.effective_message,
                            "Federasi `{}` telah mengikuti federasi `{}`",
                        ).format(fedinfo["fname"], getfed["fname"]),
                        parse_mode="markdown",
                    )
        else:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Federasi `{}` sudah mengikuti federasi `{}`.",
                ).format(fedinfo["fname"], getfed["fname"]),
                parse_mode="markdown",
            )
    else:
    ...
    if args:
        ...
        if subfed:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Federasi `{}` sudah tidak mengikuti `{}` lagi.",
                ).format(fedinfo["fname"], getfed["fname"]),
                parse_mode="markdown",
            )
            get_fedlog = sql.get_fed_log(args[0])
            if get_fedlog:
                if int(get_fedlog) != int(chat.id):
                    context.bot.send_message(
                        ...
                        tl(
                            update.effective_message,
                            "Federasi `{}` sudah tidak mengikuti `{}`",
                        ).format(fedinfo["fname"], getfed["fname"]),
                        parse_mode="markdown",
                    )
        else:
            send_message(
                ...
                tl(
                    update.effective_message,
                    "Federasi `{}` tidak mengikuti federasi `{}`.",
                ).format(fedinfo["fname"], getfed["fname"]),
                parse_mode="markdown",
            )
    else:
...
def get_myfedsubs(update, context):
    ...
    if len(getmy) == 0:
        send_message(
            ...
            tl(
                update.effective_message,
                "Federasi `{}` tidak mengikuti federasi apapun.",
        ...
        listfed = tl(
            update.effective_message, "Federasi `{}` mengikuti federasi berikut ini:\n"
        ).format(fedinfo["fname"])
        for x in getmy:
            listfed += "- `{}`\n".format(x)
        listfed += tl(
...
def __stats__():
    ...
    return tl(OWNER_ID, "{} pengguna di fbanned, pada {} federasi").format(
...
def __user_info__(user_id, chat_id):
    ...
    if fed_id:
        ...
        if int(info["owner"]) == user_id:
            text = tl(
                chat_id, "Pengguna ini adalah owner di federasi saat ini: <b>{}</b>."
            ).format(infoname)
        elif is_user_fed_admin(fed_id, user_id):
            text = tl(
                chat_id, "Pengguna ini adalah admin di federasi saat ini: <b>{}</b>."
            ).format(infoname)
...
FED_IMPORTBAN_HANDLER = CommandHandler(
    "importfbans", fed_import_bans, pass_chat_data=True, filters=Filters.user(OWNER_ID)
)
