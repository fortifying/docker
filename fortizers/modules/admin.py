...
def promote(update, context):
    ...
    try:
        ...
        if error.message == "Bot_groups_blocked":
            send_message(
                update.effective_message,
                tl(update.effective_message, "Failed to promote: Bot was locked"),
            )
        else:
            send_message(
                update.effective_message,
    ...
    send_message(
        update.effective_message, tl(update.effective_message, "Successfully promoted!")
    )
...
def pin(update, context):
    ...
    if len(args) >= 1:
        is_silent = not (
            args[0].lower() == "silent"
            or args[0].lower() == "off"
            or args[0].lower() == "mute"
        )

    if prev_message and is_group:
        try:
            context.bot.pinChatMessage(
                chat.id, prev_message, disable_notification=is_silent
            )
            if conn:
                send_message(
                    update.effective_message,
                    tl(
                        update.effective_message,
                        "I have pinned messages in the group {}",
                    ).format(chat_name),
                )
    ...
    try:
        ...
        if conn:
            send_message(
                update.effective_message,
                tl(
                    update.effective_message, "I have unpin the message in the group {}"
                ).format(chat_name),
            )
    except BadRequest as excp:
        if excp.message == "Chat_not_modified":
...
def adminlist(update, context):
    ...
    try:
        ...
        send_message(
            update.effective_message, text, parse_mode=ParseMode.MARKDOWN, quote=False
        )
...
def permapin(update, context):
    ...
    try:
        ...
        send_message(
            update.effective_message,
            tl(update.effective_message, "I don't have access to pin message!"),
        )
...
def permanent_pin_set(update, context):
    ...
    if conn:
        ...
        if not args:
            ...
            text_maker = tl(
                update.effective_message, "Permanen pin saat ini di atur: `{}`"
            ).format(bool(int(get_permapin)))
            if get_permapin:
                if chat.username:
        ...
        if prev_message == "off":
            ...
            send_message(
                update.effective_message,
                tl(update.effective_message, "Permanen pin telah di nonaktifkan!"),
            )
            return
        if "/" in prev_message:
            prev_message = prev_message.split("/")[-1]
...
def permanent_pin(update, context):
    ...
    if get_permapin and not user.id == context.bot.id:
        try:
            to_del = context.bot.pinChatMessage(
...
PERMAPIN_HANDLER = DisableAbleCommandHandler(
    "permapin", permapin, filters=Filters.group
)

INVITE_HANDLER = CommandHandler("invitelink", invite, filters=Filters.group)

PROMOTE_HANDLER = DisableAbleCommandHandler(
    "promote", promote, pass_args=True, filters=Filters.group
)
DEMOTE_HANDLER = DisableAbleCommandHandler(
    "demote", demote, pass_args=True, filters=Filters.group
)

PERMANENT_PIN_SET_HANDLER = DisableAbleCommandHandler(
