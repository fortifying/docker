...
if ENV:
    ...
    try:
        GROUP_BLACKLIST = set(
            int(x) for x in os.environ.get("GROUP_BLACKLIST", "").split()
        )
    except ValueError:
        raise Exception(
            "Your GROUP_BLACKLIST users list does not contain valid integers."
        )
    ...
    BAN_STICKER = os.environ.get(
        "BAN_STICKER",
        "CAACAgIAAxkBAAL8Z176HbDPJXrvmr_JrrNLZhTnvdhCAAI0EAACjC39B_YlpFuVGKxHGgQ",
    )
...
try:
    from fortizers.antispam import (
    ...
    def check_user(update, context, *args, **kwargs):
        ...
        if int(user.id) in SPAMMERS:
            ...
            dispatcher.bot.sendMessage(
                chat.id, "This group is in blacklist, i'm leave..."
            )
