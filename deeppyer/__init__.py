...
async def deepfry(
    ...
    *,
    ...
    if token:
        ...
        if session:
            ...
            async with aiohttp.ClientSession() as s, s.post(
                req_url, headers=headers, data=b.read()
            ) as r:
                try:
                    face_data = await r.json()
                except Exception:
        ...
        if face_data:
            ...
            eye_left_width = math.ceil(
                landmarks["eyeLeftInner"]["x"] - landmarks["eyeLeftOuter"]["x"]
            )
            eye_left_height = math.ceil(
                landmarks["eyeLeftBottom"]["y"] - landmarks["eyeLeftTop"]["y"]
            )
            eye_left_corner = (
            ...
            flare_left_size = (
                eye_left_height if eye_left_height > eye_left_width else eye_left_width
            )
            flare_left_size *= 4
            eye_left_corner = tuple(
                math.floor(x - flare_left_size / 2.5 + 5) for x in eye_left_corner
            )

            eye_right_width = math.ceil(
                landmarks["eyeRightOuter"]["x"] - landmarks["eyeRightInner"]["x"]
            )
            eye_right_height = math.ceil(
                landmarks["eyeRightBottom"]["y"] - landmarks["eyeRightTop"]["y"]
            )
            eye_right_corner = (
            ...
            flare_right_size = (
                eye_right_height
                if eye_right_height > eye_right_width
                else eye_right_width
            )
            flare_right_size *= 4
            eye_right_corner = tuple(
                math.floor(x - flare_right_size / 2.5 + 5) for x in eye_right_corner
            )
    ...
    if token and face_data:
        ...
        flare_left = flare.copy().resize(
            (flare_left_size,) * 2, resample=Image.BILINEAR
        )
        flare_right = flare.copy().resize(
            (flare_right_size,) * 2, resample=Image.BILINEAR
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--token", help="Token to use for facial recognition API."
    )
